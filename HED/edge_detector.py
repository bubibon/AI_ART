...
#parser.add_argument('--output', help='Specifies the output image path ', default='.', type=str)
...
im = args.input
imagename = im.replace("/content/drive/MyDrive/impressionist_landscapes_resized_1024", "/content/drive/MyDrive/HED_out")
...
#inverted_image.save('{}'.format(imagename))
#inverted_image.save('{}'.format(imagename))

con=np.concatenate((image,out),axis=1)
cv.imwrite('{}'.format(imagename),con)
...
print(image.shape)
#con=np.concatenate((image,out),axis=1)
#cv.imwrite('hed/{}'.format(imagename),out)